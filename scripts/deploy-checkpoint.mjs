...
const source = await readFile(resolve(process.cwd(), "contracts/BaseCheckpoint.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "BaseCheckpoint.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["BaseCheckpoint.sol"]?.BaseCheckpoint;
...
console.log(`Deploying BaseCheckpoint from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_CHECKPOINT_CONTRACT_ADDRESS to this contract address in Vercel.");
